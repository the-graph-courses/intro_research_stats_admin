...
::: practice
**Identifying Variable Types (draft)**

MCQs

1.  Education level, responses are "high", "medium", "low". Is the variable type:
...
Can you identify which variables are numeric?
...
::: practice
**Identifying Variable Types and R Data classes (draft)**

MCQs about different variables in dataset - e.g., birth date variable type is continuous (discrete), and R data class is date.
:::
...
Coding question using different categorical variables in dataset.
...
-   Ordinal: Reorder health insurance status and plot
:::
...
(copy some ideas from FoSSA quizzes)
:::
...
Coding questions using different numeric variables in dataset.
...
Now let's use {ggplot2} to plot the distribution of ages, which is recorded in the `age` column.

```{r}
# Simple histogram showing the distribution of age
ggplot(data = yaounde, 
       mapping = aes(x = age)) +
...
We can see that this is a slightly left-skewed distribution,.
...
1.  What is the "center" or "most typical" value for each continent?
2.  How do the values spread out? Are they skewed or do they have a relatively normal distribution?
:::
...
pacman::p_load(visdat, inspectdf)

# Visualize data types and missing data
vis_dat(yaounde)
